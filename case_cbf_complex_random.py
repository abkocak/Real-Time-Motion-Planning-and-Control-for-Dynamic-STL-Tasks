...
def mean_CI(data):
    ...
    length=interval[1]-mean
    return mean,length
random_list_qp=[]
random_list_avg=[]
random_list_total=[]
for rand_index in range(50):
    ...
    alpha_b=.2 #.4 -  Sequential CBF tuning parameter
    alpha_h=.98 # .9 - Holding CBF -single task with rem_time=1
    ...
    for i in range(int(hrz)):
        ...
        v_targets_all = np.vstack((-v_tar_max_all+2*v_tar_max_all*np.random.rand(1,n_tar_all),\
    			       -v_tar_max_all+2*v_tar_max_all*np.random.rand(1,n_tar_all)))# Expand to all periodic tasks over the chain
    ...
    #
        ...
        ###########################################################################    
            if c==gamma: 
                try:
                    newgamma = np.argmin(portions[gamma+1:]); # Original portions - not updated ?!?!?!
                except:
                    newgamma = 0
    ...
    random_list_qp.append(sum(elapsed_qp)/hrz)
    random_list_avg.append(sum(elapsed_run)/hrz)
    random_list_total.append(sum(elapsed_run))

print({'Average QP Soln Time':mean_CI(random_list_qp)})
print({'Average Online Runtime':mean_CI(random_list_avg)})
print({'Total Online Runtime':mean_CI(random_list_total)})
