...
# Trajectory parameters for the targets
coeffs=np.array([[ -pi, pi, 0],
                 [0, pi*2, -pi*2],
                 [-1*pi, .5*pi, 0],
                 [0, pi*1, -pi*2],
                 [0,  pi*1.15, -pi*2]])
coeffs_disj=np.copy(coeffs)
coeffs_disj[alt_inds>0] = [-pi, .75*pi, 0]

coeffs_all=np.vstack((coeffs,coeffs_disj[alt_inds>0]))
...
b_minus_dist = Function('b_minus_dist',[vc,rc],[rc*umax-vc*rc])# -dist_p_cir[xc,vc,rc,xp]
...
for i in range(int(hrz)):
    ...
    theta=(i+1)/hrz*coeffs_all[:,0]+coeffs_all[:,1]+coeffs_all[:,2]*sin((i+1)/hrz*pi)

    v_targets_all = [np.cos(theta)*v_tar_max_all, np.sin(theta)*v_tar_max_all] # Expand to all periodic tasks over the chain
    ...
    for j in range(c+1,gamma+1):
        ...
        const += dist_cir_cir(xc_v,vc_v,rc_v,xc_pw_v,vc_pw_v,rc_pw_v) #double(subs(b_pair));
